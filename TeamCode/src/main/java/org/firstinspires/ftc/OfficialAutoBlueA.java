...
@Autonomous
public class OfficialAutoBlueA extends LinearOpMode {
...
//drop the jewel knocker
/*
 while(Motor5.getCurrentPosition()<55&&opModeIsActive()){ 
     Motor5.setPower(0.04);
 }
 Motor5.setPower(0);
 //jewel knocker motor pushes back against gravity to hold pos
 Motor5.setPower(-0.03);
 //scan jewel color
 Timer.reset();
 while(Timer.seconds()<2){
     ...
     if((colorSensor.blue()>0)&&(colorSensor.red()==0)){
         //if blue
         Timer.reset();
         //move out a little to knock blue 
         while(Timer.seconds()<.5){
             Motor4.setPower(0.1);
             Motor3.setPower(-0.1);
         }
         //bring back jewel knocker
         while(Motor5.getCurrentPosition()>0){
         Motor5.setPower(-0.04);
         }
         while(Timer.seconds()<.5){
             Motor4.setPower(-0.1);
...
Timer.reset();
 ...
 Timer.reset();
 // drive off balance board and gjust past center
    while(Timer.seconds()<2.7 && opModeIsActive()){ 
        Motor4.setPower(0.2);
        Motor3.setPower(-0.2);
...
/*
        while(integratedZ>-100&&opModeIsActive()){
    ...
    Motor2.setPower(-0.2);
        ...
        while(integratedZ>-179&&opModeIsActive()){
             integratedZ = modernRoboticsI2cGyro.getIntegratedZValue();
    ...
    Motor2.setPower(-0.1);  
    Timer.reset();
        while(integratedZ!=-180&&opModeIsActive()&&Timer.seconds()<2){
            if(integratedZ>-180){
    Motor4.setPower(-0.08);
    Motor3.setPower(-0.08);
    Motor1.setPower(-0.08);
    ...
    Motor1.setPower(0);
    Motor2.setPower(0);
    telemetry.addData("DSensor", rangeSensor.rawUltrasonic());
                    telemetry.update();
        }
    
    while(rangeSensor.rawUltrasonic()>39){
        telemetry.addData("DSensor", rangeSensor.rawUltrasonic());
                    telemetry.update();
        Motor1.setPower(-0.1);
        Motor2.setPower(0.1);
...
while(runtime.seconds()<5){
  ...
  /*          
            if(VuMark == "RIGHT"){
                int numOfSpikes = 0;
                while(numOfSpikes < 2&&opModeIsActive()){
                    Motor4.setPower(-0.1);
                    Motor3.setPower(0.1);
                    
                   
                    if(rangeSensor.rawUltrasonic()<40){
                ...
                while(numOfSpikes < 2 &&opModeIsActive()){
                    ...
                    Motor4.setPower(0.1);
                    Motor3.setPower(-0.1);
                    telemetry.addData("Spikey",numOfSpikes);
                    telemetry.addData("DSensor", rangeSensor.getDistance(DistanceUnit.CM));
                    telemetry.update();
                    if(rangeSensor.rawUltrasonic()<40&&opModeIsActive()){
                        while(rangeSensor.rawUltrasonic()<40&&opModeIsActive()){
                       //empty
                        }
                        //found spike
            ...
            */
                // VuMark is either CENTER or it was not read
                int numOfSpikes = 0;
                while(numOfSpikes < 1&&opModeIsActive()){
                    Motor4.setPower(0.1);
                    Motor3.setPower(-0.1); //move right until spike is detected
                    ...
                    Timer.reset();
                    while(Timer.seconds()<1&&opModeIsActive()){ //seconds needs testing
                        Motor4.setPower(-0.1);
                    ...
                    Motor3.setPower(0);
                    Timer.reset();
                    while(Timer.seconds()<1.5&&opModeIsActive()){ //seconds needs testing
                        Motor2.setPower(0.1);
                        Motor1.setPower(-0.1); //glyph goes in
                    }
                    Motor2.setPower(0);
